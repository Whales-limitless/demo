				...
				<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
					...
					<table id="example" class="table table-striped table-hover" style="width:100%">
						...
						<tbody>
							...
							$sql = $connect->query("SELECT DRIVER, COUNT(ORDNO) AS CORDNO, SUM(DISTANT) AS SDISTANT, SUM(RETAIL) AS SRETAIL FROM `del_orderlist` WHERE (STATUS = 'D' OR STATUS = 'C') AND DELDATE BETWEEN '".$sdate."' AND '".$edate."' $ttxt1 $ttxt2 GROUP BY DRIVERCODE");
						...
						<tfoot>
							<?php
							$sql12 = $connect->query("SELECT COUNT(ORDNO) AS CORDNO, SUM(DISTANT) AS SUMDISTANT, SUM(RETAIL) AS SUMRETAIL FROM `del_orderlist` WHERE (STATUS = 'D' OR STATUS = 'C') AND DELDATE BETWEEN '".$sdate."' AND '".$edate."' $ttxt1 $ttxt2 ");
							if($sql12->num_rows > 0){
