...
$info = $connect->query("SELECT * FROM orderlist WHERE ORDNO = '$getorderno'");
$inforow = $info->fetch_assoc();


$custcode = $inforow["CUSTOMERCODE"];
 ...
 <button type="button" class="btn btn-sm btn-outline-secondary">Export</button>
					...
					<div class="row">
						<div class="col-md-2">
							...
							<input type="date" class="form-control" name="deldate" id="deldate" value="<?php echo $inforow["DELDATE"]; ?>" >
						</div>
						<div class="col-md-1">
   ...
   <div class="col-md-2">
 <br>
 <select class="driver w-100" name="driver" id="driver">
  <option value=""></option>
  <?php
$driver = $connect->query("SELECT * FROM driver ORDER BY NAME ASC");
  ...
  -->
						<div class="col-md-2">
							...
							<select class="customer w-100" name="customer" id="customer">
								...
								while($driverrow = $driver->fetch_assoc()){
								?>
								<option value="<?php echo $driverrow["CODE"]; ?>" <?php if($driverrow["CODE"] == $inforow["CUSTOMERCODE"]){echo "selected";} ?> ><?php echo $driverrow["NAME"]; ?></option>
								<?php
								}
								?>
							</select>
						...
						<div class="col-md-3">
							...
							<input type="text" name="remark" id="remark" value="<?php echo $inforow["REMARK"]; ?>" class="form-control">
						</div>
					</div>
					<br>
					<div class="row">
						<div class="col-md-1">
							...
							<input type="text" name="location" id="location" class="form-control" value="<?php echo $inforow["LOCATION"]; ?>" readonly>
						</div>
						<div class="col-md-3">
							<label>Address</label>
							<input type="text" name="address" id="address" class="form-control" value="<?php echo $custaddress; ?>" readonly>
						</div>
						<div class="col-md-1">
							<label>Distant</label>
							<input type="text" name="distant" id="distant" class="form-control" value="<?php echo $inforow["DISTANT"]; ?>" readonly>
						</div>
						<div class="col-md-1">
							<label>Commission</label>
							<input type="text" name="retail" id="retail" class="form-control" value="<?php echo $inforow["RETAIL"]; ?>"  readonly>
						</div>
									...
									<table class="table table-bordered table-sm" id="option_table" style="width:100%">
										<thead>
											...
											<tr>
												...
												<th><button type="button" onclick="add()" class="btn btn-primary btn-sm w-100">Save</button></th>
											</tr>
										</thead>
										<tbody>
											<?php
						$num = 0;
						$temp = $connect->query("SELECT * FROM orderlistdesc WHERE ORDERNO = '$getorderno'");
