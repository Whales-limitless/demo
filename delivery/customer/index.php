...
<html lang="en">
 ...
 <button type="button" class="btn btn-sm btn-outline-secondary">Export</button>
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<div class="row">
										<div class="col">
											<div class="mb-3">
												<select class="location" name="a4" id="a4" style="width:100%;">
													<option value=""></option>
													<?php
													$driver = $connect->query("SELECT * FROM location ORDER BY NAME ASC");
													while($driverrow = $driver->fetch_assoc()){
													?>
													<option><?php echo $driverrow["NAME"]; ?></option>
													<?php
													}
													?>
												</select>
									...
									<div class="row">
										<div class="col-6">
											<div class="mb-3">
												<label for="a5" class="form-label">Phone</label>
												<input type="number" class="form-control" id="a5">
											</div>
										</div>
									</div>
					...
					<table id="example" class="table table-striped" style="width:100%">
						...
						<tbody>
							<?php
							$sql = $connect->query("SELECT * FROM customer ORDER BY NAME ASC");
							...
							<tr>
								...
								<td>
									<div class="btn-group" role="group" aria-label="Basic mixed styles example">
										<button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#editcustomer<?php echo $row["ID"]; ?>">Edit</button>
										<button type="button" class="btn btn-danger" onclick="if (confirm('Delete selected item?')){del(<?php echo $row["ID"]; ?>);}else{};">Delete</button>
									</div>
								</td>
							</tr>
							<div class="modal fade" id="editcustomer<?php echo $row["ID"]; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="exampleModalLabel">Edit Customer(<?php echo $row["ID"]; ?>)</h5>
											<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
										</div>
										<div class="modal-body">
											...
											<div class="row">
												<div class="col">
													<div class="mb-3">
														<select class="location" name="e4" id="e4-<?php echo $row["ID"]; ?>" style="width:100%;">
													<option value=""></option>
													<?php
													$driver = $connect->query("SELECT * FROM location ORDER BY NAME ASC");
													while($driverrow = $driver->fetch_assoc()){
													?>
													<option <?php if($row["LOCATION"] == $driverrow["NAME"]){echo "selected";} ?>><?php echo $driverrow["NAME"]; ?></option>
													<?php
													}
													?>
												</select>
											...
											<div class="row">
												<div class="col-6">
													<div class="mb-3">
														<label for="a5" class="form-label">Phone</label>
														<input type="number" class="form-control" id="e5-<?php echo $row["ID"]; ?>" value="<?php echo $row["HP"]; ?>">
													</div>
												</div>
											</div>
							...
							<script>
								function edit<?php echo $row["ID"]; ?>(id){
									...
									var location = document.getElementById("e4-<?php echo $row["ID"]; ?>").value;
									var hp = document.getElementById("e5-<?php echo $row["ID"]; ?>").value;
		...
		<script>
			function add(){
				...
				var location = document.getElementById("a4").value;
				var hp = document.getElementById("a5").value;
