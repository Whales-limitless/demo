...
$currentPage = 'report_sales_staff';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Sales by Staff Report</title>
...
.staff-subheader { background: #f0f0f0; padding: 8px 16px; font-weight: 600; font-size: 13px; border-bottom: 1px solid #d1d5db; display: flex; justify-content: space-between; align-items: center; }
.staff-subheader .staff-stats { font-size: 12px; font-weight: 400; color: var(--text-muted); }
...
<div class="page-content">
    <div class="page-header">
        <h1><i class="fas fa-user-tie" style="color:var(--primary);margin-right:8px;"></i>Sales by Staff Report</h1>
    </div>
...
function generateReport() {
    var action = reportMode === 'detailed' ? 'sales_by_staff_detailed' : 'sales_by_staff';
    var postData = {
        action: action,
...
function renderTable(rows) {
    ...
    var html = '<table id="reportTable" class="table table-striped table-sm" style="width:100%;font-size:13px;">' +
        '<thead><tr><th>No</th><th>Staff</th><th class="text-end">Orders</th><th class="text-end">Qty Sold</th><th class="text-end">Unique Products</th><th>First Sale</th><th>Last Sale</th></tr></thead><tbody>';
    ...
    rows.forEach(function(r, i) {
        ...
        totalOrders += orders;
        totalQty += qty;
        var firstSale = r.first_sale ? new Date(r.first_sale + 'T00:00:00').toLocaleDateString('en-GB') : '';
        var lastSale = r.last_sale ? new Date(r.last_sale + 'T00:00:00').toLocaleDateString('en-GB') : '';
        html += '<tr>' +
            '<td>' + (i+1) + '</td>' +
            '<td>' + escHtml(r.staff_name || 'Unknown') + '</td>' +
            '<td class="text-end">' + orders + '</td>' +
            '<td class="text-end fw-bold">' + qty.toFixed(2) + '</td>' +
            '<td class="text-end">' + (r.unique_products || 0) + '</td>' +
            '<td>' + escHtml(firstSale) + '</td>' +
            '<td>' + escHtml(lastSale) + '</td>' +
            '</tr>';
    });

    html += '</tbody><tfoot><tr style="font-weight:700;"><td colspan="2">TOTAL</td><td class="text-end">' + totalOrders + '</td><td class="text-end">' + totalQty.toFixed(2) + '</td><td colspan="3"></td></tr></tfoot></table>';
    document.getElementById('reportContent').innerHTML = html;

    document.getElementById('summaryCards').style.display = 'flex';
    document.getElementById('sumStaff').textContent = rows.length;
...
function renderDetailedTable(rows) {
    ...
    rows.forEach(function(r) {
        ...
        if (!branches[bCode]) branches[bCode] = { name: bName, staff: {}, qty: 0, orders: {} };
        var staff = r.staff_name || 'Unknown';
        allStaff[staff] = true;
        if (!branches[bCode].staff[staff]) branches[bCode].staff[staff] = { items: [], qty: 0, orders: {} };
        var qty = parseFloat(r.qty) || 0;
        branches[bCode].staff[staff].items.push(r);
        branches[bCode].staff[staff].qty += qty;
        branches[bCode].staff[staff].orders[r.order_no] = true;
    ...
    var html = '<div class="print-header" style="display:none;">Sales by Staff Report (Detailed) | ' + escHtml(dateRange) + '</div>' +
