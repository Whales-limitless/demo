...
<div class="page-content">
    ...
    <div class="table-card">
        <div class="table-toolbar">
            ...
            <button class="btn-add" onclick="openMonthlyExportModal();" style="padding:7px 14px;font-size:12px;"><i class="fas fa-calendar-alt"></i> Monthly Report</button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-calendar-alt" style="color:var(--primary);margin-right:6px;"></i>Monthly Stock Loss Report</h5>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn-export btn-export-pdf" id="btnMonthlyPdf" onclick="exportMonthlyPDF();" disabled style="padding:8px 16px;"><i class="fas fa-file-pdf"></i> Export PDF</button>
                <button type="button" class="btn-export btn-export-excel" id="btnMonthlyExcel" onclick="exportMonthlyExcel();" disabled style="padding:8px 16px;"><i class="fas fa-file-excel"></i> Export Excel</button>
            </div>
        </div>
    </div>
</div>
...
function loadMonthlySummary() {
    ...
    $.ajax({
        type: 'POST', url: 'stock_loss_ajax.php', data: { action: 'monthly_summary', month: month, year: year }, dataType: 'json',
        success: function(data) {
            monthlyData = data;
            if (!data.items || data.items.length === 0) {
                preview.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted);"><i class="fas fa-info-circle"></i> No stock loss records for ' + monthNames[month - 1] + ' ' + year + '</div>';
                return;
            }
            ...
            for (var r in reasons) {
                badges += '<span class="badge-reason badge-' + escHtml(r) + '" style="margin-right:6px;">' + escHtml(r) + ': ' + reasons[r].count + ' (' + reasons[r].qty + ' qty)</span>';
            }
...
function buildMonthlyExportHtml(useBase64) {
    ...
    for (var r in reasons) {
        html += '<tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:6px 14px;font-weight:600;">' + escHtml(r) + '</td><td style="padding:6px 14px;text-align:center;">' + reasons[r].count + '</td><td style="padding:6px 14px;text-align:center;">' + reasons[r].qty + '</td></tr>';
    }
    html += '</tbody></table>';
    ...
    items.forEach(function(item, i) {
        ...
        html += '<td style="padding:6px 8px;">' + escHtml(item.SDATE || '') + '</td>';
        html += '<td style="padding:6px 8px;">' + imgTag + '</td>';
        html += '<td style="padding:6px 8px;font-weight:600;">' + escHtml(item.PDESC || '') + '</td>';
        html += '<td style="padding:6px 8px;text-align:center;font-weight:700;">' + qty + '</td>';
        html += '<td style="padding:6px 8px;">' + escHtml(item.LOSS_REASON || '') + '</td>';
        html += '<td style="padding:6px 8px;">' + escHtml(item.REMARK || '') + '</td>';
        html += '<td style="padding:6px 8px;">' + escHtml(item.USER || '') + '</td>';
        html += '</tr>';
    });
