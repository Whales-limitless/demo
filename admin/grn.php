    ...
    <div class="table-card">
        <div class="table-toolbar">
            <div class="search-box">
                ...
                <input type="text" id="searchInput" placeholder="Search GRN number, supplier, PO...">
            </div>
            <div class="item-count" id="itemCount">0 GRN(s)</div>
        </div>
...
<div class="modal fade" id="viewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewTitle"><i class="fas fa-dolly"></i> GRN Detail</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<script>
var receiveModal = null, viewModal = null, productSearchModal = null;
var grnLineIndex = 0;
...
function loadGRNs() {
    $.ajax({
        type: 'POST', url: 'grn_ajax.php', data: { action: 'list' }, dataType: 'json',
...
function renderGRNTable(grns) {
    ...
    if (grns.length === 0) {
        ...
        document.getElementById('itemCount').textContent = '0 GRN(s)';
        return;
    }
    ...
    body.innerHTML = html;
    document.getElementById('itemCount').textContent = grns.length + ' GRN(s)';
}
...
function loadReceivablePOs() {
    $.ajax({
        type: 'POST', url: 'grn_ajax.php', data: { action: 'list_receivable_pos' }, dataType: 'json',
        success: function(data) {
...
function loadSuppliers() {
    $.ajax({
        type: 'POST', url: 'grn_ajax.php', data: { action: 'list_suppliers' }, dataType: 'json',
        success: function(data) {
...
function loadPOItems() {
    ...
    $.ajax({
        type: 'POST', url: 'grn_ajax.php', data: { action: 'get_po_items', po_id: poId }, dataType: 'json',
        success: function(data) {
            if (data.error) { Swal.fire({ icon: 'error', text: data.error }); return; }
            var po = data.po;
...
// UOM conversion cache: { barcode: { from_uom: { to_uom, factor } } }
...
function lookupConversion(lineIdx, barcode, fromUom) {
    $.post('grn_ajax.php', { action: 'uom_conversion_lookup', barcode: barcode, from_uom: fromUom }, function(data) {
        ...
        if (data.found) {
            ...
            // No conversion: receive UOM = inventory UOM
...
function loadProducts(append) {
    ...
    psmSearchXhr = $.ajax({
        type: 'POST', url: 'grn_ajax.php', data: { action: 'search_products', q: psmCurrentQuery, offset: psmCurrentOffset }, dataType: 'json',
        success: function(data) {
            psmSearchXhr = null;
            ...
            products.forEach(function(p) {
                ...
                html += '<div class="psm-card" data-barcode="' + escHtml(p.barcode) + '" data-name="' + escHtml(p.name) + '" data-uom="' + escHtml(p.uom || '') + '" data-rack="' + escHtml(p.rack || '') + '" onclick="selectProductFromCard(this);">';
                html += imgHtml;
...
function viewGRN(id) {
    $.ajax({
        type: 'POST', url: 'grn_ajax.php', data: { action: 'get', id: id }, dataType: 'json',
        success: function(data) {
            if (data.error) { Swal.fire({ icon: 'error', text: data.error }); return; }
            var grn = data.grn;
            ...
            document.getElementById('viewTitle').innerHTML = '<i class="fas fa-dolly"></i> ' + escHtml(grn.grn_number);
