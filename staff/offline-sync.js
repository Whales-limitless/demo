...
var OfflineSync = (function() {
  ...
  // Encode like PHP's urlencode() - same as encodeURIComponent but spaces become +
  // This ensures prefetch URLs exactly match the hrefs generated by PHP urlencode()
  ...
  function downloadAll(userType, onProgress) {
    ...
    // Staff pages to prefetch for offline use
    // NOTE: del_work.php, del_vieworder.php, del_sign.php are excluded here because
    // they redirect without query params. Per-order versions with params are added below.
    var pages = [
      '/staff/index.php',
      '/staff/account.php',
      '/staff/category.php',
    ...
    return downloadOfflineData(function() {})
      .then(function(data) {
        ...
        // Prefetch individual order pages with specific IDs/ordno params
        // Uses phpUrlencode() to match PHP urlencode() encoding in links,
        // ensuring cached URLs exactly match navigation URLs for SW cache hits.
        if (data && data.orders) {
          for (var i = 0; i < data.orders.length; i++) {
            var orderId = data.orders[i].ID;
            var ordno = data.orders[i].ORDNO;
            var encodedOrdno = phpUrlencode(ordno);
            pages.push('/staff/del_work.php?id=' + orderId);
            pages.push('/staff/del_vieworder.php?ordno=' + encodedOrdno);
            pages.push('/staff/del_sign.php?ordno=' + encodedOrdno + '&id=' + orderId);
          }
