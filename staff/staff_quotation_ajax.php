...
function generateQuotationNumber($connect) {
    $prefix = 'QT-' . date('Ym') . '-';
    $result = $connect->query("SELECT `quotation_number` FROM `quotation` WHERE `quotation_number` LIKE '$prefix%' ORDER BY `quotation_number` DESC LIMIT 1");
    $next = 1;
...
// ==================== PO LIST ====================
    ...
    $sql = "SELECT po.*, s.name AS supplier_name FROM `quotation` po LEFT JOIN `supplier` s ON po.supplier_id = s.id WHERE $where ORDER BY po.id DESC LIMIT 100";
...
// ==================== CREATE PO ====================
    ...
    $itemStmt = $connect->prepare("INSERT INTO `quotation_item` (`quotation_id`,`barcode`,`product_desc`,`qty_ordered`,`unit_cost`,`uom`) VALUES (?,?,?,?,0,?)");
...
// ==================== UPDATE PO ====================
    ...
    $chk = $connect->query("SELECT `status` FROM `quotation` WHERE `id` = $id LIMIT 1");
    ...
    $stmt = $connect->prepare("UPDATE `quotation` SET `supplier_id`=?,`order_date`=?,`expected_date`=?,`total_amount`=0,`remark`=? WHERE `id`=?");
    $stmt->bind_param("isssi", $supplierId, $orderDate, $expectedDateVal, $remark, $id);
    $stmt->execute();
    $stmt->close();

    $connect->query("DELETE FROM `quotation_item` WHERE `quotation_id` = $id");
...
// ==================== GET PO DETAIL ====================
    ...
    $itemResult = $connect->query("SELECT poi.*, p.img1 AS product_image FROM `quotation_item` poi LEFT JOIN `PRODUCTS` p ON poi.barcode = p.barcode WHERE poi.quotation_id = $id ORDER BY poi.id ASC");
    $items = [];
    if ($itemResult) {
